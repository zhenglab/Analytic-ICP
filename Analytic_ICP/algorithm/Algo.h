...
*/
template<class TT1>
void __declspec(dllexport) AnalyticT2d(TT1(*v)[2], TT1(*mo)[2], int num, int deg)
{
...
// C^∞ non-analytic bump
inline double bump(double r, double sigma) {
...
// Normalized bicentric weights
inline void w_bump_normed(const double y[3], const double c1[3], const double c2[3],
	double sigma, double &w1, double &w2) {
	auto norm = [](const double a[3], const double b[3]) {
		double dx = a[0] - b[0], dy = a[1] - b[1], dz = a[2] - b[2];
		return sqrt(dx*dx + dy*dy + dz*dz);
	};
	double b1 = bump(norm(y, c1), sigma);
	double b2 = bump(norm(y, c2), sigma);
	double s = b1 + b2;
	if (s <= 1e-16) { w1 = 0.5; w2 = 0.5; }
	else { w1 = b1 / s; w2 = b2 / s; }
}
...
inline void AnalyticT3d(double(*v)[3], double(*mo)[3], int num, int deg)
{
	...
	// Randomly initialize the coefficients for orders 0, …, deg.
	for (int i = 0; i <= deg; ++i)
	{
		for (int j = 0; j != MAXMATSIZE; ++j)
		{
			mat_set[i][j] = 0.14 * rand() / RAND_MAX - 0.07;
			fprintf(fp, "%f\n", mat_set[i][j]);
		}
	}
	fclose(fp);

	// Set the first-order term to 1.
	mat_set[1][0] = 1;
	mat_set[1][4] = 1;
	mat_set[1][8] = 1;

	// Keep these three variables (even if unused).
	double *pTerm = new double[3];
	int sum = 0;
	int item_num = 0;

	// -----------------------
	// Piecewise second-order mapping with dual centers + Gaussian weighting
	// (enabled only when deg >= 2)
	// -----------------------
	// Choose two centers: the points with the minimum x / maximum x
	int idx_minx = 0, idx_maxx = 0;
	for (int i = 1; i < num; ++i) {
		if (v[i][0] < v[idx_minx][0]) idx_minx = i;
		if (v[i][0] > v[idx_maxx][0]) idx_maxx = i;
	}
	double c1[3] = { v[idx_minx][0], v[idx_minx][1], v[idx_minx][2] };
	double c2[3] = { v[idx_maxx][0], v[idx_maxx][1], v[idx_maxx][2] };

	auto sq = [](double x) { return x * x; };
	double d2c = sq(c1[0] - c2[0]) + sq(c1[1] - c2[1]) + sq(c1[2] - c2[2]);
	double sigma2 = (d2c > 1e-12) ? (0.25 * d2c) : 1.0; // sigma = 0.5 * dist; avoid division by zero

	// Backup the original second-order coefficients
	memcpy(o2, mat_set[2], sizeof(double) * MAXMATSIZE);

	// Two fixed sets of second-order coefficients (Q1/Q2), ensuring both branches share the same mapping construction
	coefmat Q1, Q2, Qmix;
	for (int j = 0; j < MAXMATSIZE; ++j) {
		double d1 = 0.06 * rand() / RAND_MAX - 0.03; 
		double d2 = 0.06 * rand() / RAND_MAX - 0.03; 
		Q1[j] = o2[j] + d1;
		Q2[j] = o2[j] + d2;
	}

	auto w_gauss = [&](const double y[3], const double c[3]) {
		double d2 = sq(y[0] - c[0]) + sq(y[1] - c[1]) + sq(y[2] - c[2]);
		return exp(-d2 / (2.0 * sigma2));
	};

	// Pointwise evaluation: for each point, generate the second-order coefficients using the same formula
	// Q(y) = w1 * Q1 + w2 * Q2, then apply the transform
	for (int i = 0; i != num; ++i)
	{
		// Constant term (0th order)
		mo[i][0] = mat_set[0][0];
		mo[i][1] = mat_set[0][1];
		mo[i][2] = mat_set[0][2];
		...
		if (deg >= 2) {
			double w1, w2;
			w_bump_normed(v[i], c1, c2, /*sigma=*/sqrt(sigma2), w1, w2);

			for (int j = 0; j < MAXMATSIZE; ++j) {
				Qmix[j] = w1 * Q1[j] + w2 * Q2[j];
			}
			// Write the second-order coefficients for this point to mat_set[2], then apply the transformation
			memcpy(mat_set[2], Qmix, sizeof(double) * MAXMATSIZE);
		}
		// If deg < 2, keep mat_set as initialized (no second-order coefficients)

		Analyt3dTrans(mat_set, deg, v[i], mo[i]);
	}

	// Restore second-order coefficients (optional)
	memcpy(mat_set[2], o2, sizeof(double) * MAXMATSIZE);
