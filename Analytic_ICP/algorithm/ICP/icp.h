...
public:
  ...
  Icp(Vector3d *&ps, int &p_num, int iterCount);
  Icp(Vector3d *&ps, int &p_num);

  Icp(Vector2d *&ps, int &p_num, int iterCount);
  Icp(Vector2d *&ps, int &p_num);
  ...
  void fit(double *T, const int T_num, MatrixXd &R, VectorXd &t, vector<int> &fit_set, const double indist);

  void fit(Vector2d *&ps, int &p_num, MatrixXd &R, VectorXd &t, vector<int> &fit_set, const double indist);
  void fit(Vector3d *&ps, int &p_num, MatrixXd &R, VectorXd &t, vector<int> &fit_set, const double indist);

private:
  
  // iterative fitting
	void fitIterate(double *&T, const int &T_num, MatrixXd &R, VectorXd &t, const vector<int> active, vector<int> &fit_set);
  
  // inherited classes need to overwrite these functions
	virtual double fitStep(double *T, const int T_num, MatrixXd &R, VectorXd &t, const vector<int> active, vector<int> &fit_set) = 0;
	virtual void getInliers(vector<int> &fit_set, double *T, const int T_num, const MatrixXd &R, const VectorXd &t, const double indist) = 0;
...
protected:
  ...
  void pointset2T(double *T, Vector2d *ps, const int p_num);
  void pointset3T(double *T, Vector3d *ps, const int p_num);
};
