...
DLL_PUREENTITY bool carving_initialize()
{
	hDll = LoadLibrary(L"Analytic_ICP.dll");
	...
	, const char *moving_output)
{
	...
	data_from_path(fixed, f_num, fixed_path);
	data_from_path(moving, m_num, moving_path);

	memcpy(moving_prev, moving, sizeof(double3)*m_num);

	double result;
	result = ftransf3d(moving, m_num, fixed, f_num, 8, 0.02);
	...
	, unsigned char *bmp_qst, int r, int c)
{
	...
	double result;
	result = ftransform(moving, m_num, fixed, f_num, 8);
	...
	Data2bmp(mat, merging + f_num + m_num, m_num, 0, 0, 0);
...
DLL_PUREENTITY void analytic_points_map(const char *fixed_path
	, unsigned char *bmp_qst, int r, int c)
{
	double2 moving[MAX_VERTEX_NUM] = { 0 };
	int f_num = 0;

	double2 fixed[MAX_VERTEX_NUM] = { 0 };
	double2 fixed_c[MAX_VERTEX_NUM] = { 0 };

	data_from_path(moving, f_num, fixed_path);
...
DLL_PUREENTITY void analytic_3d_map(const char *fixed_path
	, unsigned char *bmp_qst, int r, int c)
{
	double3 moving[MAX_VERTEX_NUM] = { 0 };
	int ids[MAX_VERTEX_NUM] = { 0 };
	int f_num = 0;

	double3 fixed[MAX_VERTEX_NUM] = { 0 };
	double3 fixed_c[MAX_VERTEX_NUM] = { 0 };
